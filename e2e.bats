...
@test "Reject service account with risky permissions" {
  ...
  echo "output = ${output}"
	...
	[ $(expr "$output" : '.*"Cannot use service account '\''system:serviceaccount:default:my-service-account'\'' with permissions to perform create /pods in namespace '\'''\''.*') -ne 0 ]
}
...
@test "With no service account, should use the default service account" {
  ...
  echo "output = ${output}"
	...
	[ "$status" -eq 0 ]
	[ $(expr "$output" : '.*"allowed":false.*') -ne 0 ]
	[ $(expr "$output" : '.*"Cannot use service account '\''system:serviceaccount:mynamespace:default'\'' with permissions to perform create /pods in namespace '\'''\'': plugin x blocked this operation.*') -ne 0 ]
}
